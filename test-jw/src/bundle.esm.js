...
var _default$1 = tracking.default = trackerFactoryDataLayer;
...
function addRunTimeParams(trackingParams) {
  var trackDataObject = __assign$1({}, trackingParams);

  trackDataObject[PROPERTY_NAMES.VIDEO_AUTO_PLAY_STATE] = getAutoPlayState();
  trackDataObject[PROPERTY_NAMES.VIDEO_VOLUME_LEVEL] = getVideoVolume(); // Josh
  ...
  trackDataObject[PROPERTY_NAMES.VIDEO_IS_VIEWABLE] = getIsCurrentlyViewable(); // ... TODO add them all here
...
var CONTROLS_CATEGORY = 'controls';
var baseVideoTracker = _default$1(baseTrackParams);
function trackerWithNewCategory(category) {
  return baseVideoTracker.extend({
    categoryPrefixer: function (cat) {
      return category + cat;
    }
  });
}
var CATEGORIES = {
  BASE_VIDEO_PLAYER: 'FandomVideoPlayer: mobile-article-video-',
  MOBILE_ARTICLE_VIDEO_PLAYER: 'FandomVideoPlayer: mobile-article-video-',
  DESKTOP_ARTICLE_VIDEO_PLAYER: 'FandomVideoPlayer: desktop-article-video-',
  JW_PLAYER: 'FandomVideoPlayer: ' // Generic Events

};
var baseVideoPlayerTracker = trackerWithNewCategory(CATEGORIES.BASE_VIDEO_PLAYER);
trackerWithNewCategory(CATEGORIES.MOBILE_ARTICLE_VIDEO_PLAYER);
trackerWithNewCategory(CATEGORIES.DESKTOP_ARTICLE_VIDEO_PLAYER);
var jwPlayerVideoTracker = trackerWithNewCategory(CATEGORIES.JW_PLAYER);
...
timings.differenceFromRoot = timings.difference = timings.recordMultiple = timings.getTimeOf = timings.resetTimings = void 0;
const env_1 = env;
...
var _default = timings.default = record;
...
};var AD_TIME_CATEGORY = 'ad';
var AD_ACTION = 'ad-playing';
var VIDEO_TIME_CATEGORY = 'video';
var VIDEO_TIME_ACTION = 'playing';
function addBaseTrackingEvents(playerInstance) {
  var playList = playerInstance.getPlaylistItem();
  var mediaId = playList.mediaid; // Add MediaId to the dataLayer object to be used later

  var tracker = jwPlayerVideoTracker.extend({
    mediaId: mediaId
  }); // Add events
  ...
  .on(JWEvents.PAUSE, function (event) {
    ...
    tracker({
      ...
      label: "volume-level-".concat(event.volume),
      value: event.volume
    });
    ...
    tracker.click({
      category: CONTROLS_CATEGORY,
      label: "full-screen-".concat(event.fullscreen)
    });
  }).on(JWEvents.SEEK, function (event) {
    tracker.click({
      category: CONTROLS_CATEGORY,
      label: "seek | ".concat(event.position, " | ").concat(event.offset),
      value: event.offset
    });
  }) // Errors
  .on(JWEvents.ERROR, function (event) {
    ...
    tracker({
      action: 'error',
      category: "".concat(event.code, " | ").concat(event.message.slice(0, 20)),
...
var JwPlayerWrapper = function (_a) {
  ...
  var initPlayer = function (elementId, playerUrl) {
    ...
    var script = document.createElement('script');
    jwPlayerVideoTracker({
      label: 'video-id-here',
      action: 'loading-scripts'
    });
    script.async = true;
